...
public class SearchSteps {
    ...
    public void aListOfHomesThatMatchesSpecificationsShouldBeReturnedAndPrintedOnTheConsole(String result) {
        String searchString="";
        searchResults = (ArrayList<Home>)searchI.search(arr);
    ...
    @And("An Email with a list of search results will be sent to {string} customer email")
    public void anEmailWithAListOfSearchResultsWillBeSentToCustomerEmail(String email) {
        EmailService mockedEmail = mock(EmailService.class);
        mockedEmail.sendMail(email,searchResults);
        verify(mockedEmail,times(1)).sendMail(email,searchResults);
